...
@WebServlet(name = "EmailListCtl", urlPatterns = { "/ctl/EmailListCtl" })
public class EmailListCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			List<EmailBean> list = model.search(bean, pageNo, pageSize);
			List<EmailBean> next = model.search(bean, pageNo + 1, pageSize);
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					EmailBean deletebean = new EmailBean();
					for (String id : ids) {
						deletebean.setId(DataUtility.getLong(id));
						model.delete(deletebean);
						ServletUtility.setSuccessMessage("Investor is deleted successfully", request);
					}
				} else {
	...
	protected String getView() {
		...
		return ORSView.EMAIL_LIST_VIEW;
	}
